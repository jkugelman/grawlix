...
**Don't smoke-test by running `python -m http.server`.** Serving the module graph locally only verifies that the filesystem can read it, which is theatre. For real verification, syntax-check changed JS modules with `node --check <file>` (the pre-approved `check-syntax.js` is a classic-script parser and can't read ESM), read the diff carefully, and say "I can't visually verify" when that's the truth — visual inspection requires the user's browser. Two test tiers: the Playwright browser suite ([`tests/browser/`](tests/browser/), `npm run test:browser`) covers user-visible behavior; a `node:test` unit tier ([`tests/unit/`](tests/unit/), `npm run test:unit`) covers pure logic by importing the engine/data modules directly. `npm test` is the everyday gate: unit tier, then **chromium only** against the bundled `dist` (~2 min). `npm run test:all` adds firefox and webkit (~12 min); run it before a push that touches storage, the File System Access API, workers, or rendering geometry. CI runs all three engines on every push and `deploy` needs `test`, so an engine-specific break blocks the release. See [`docs/testing.md`](docs/testing.md) for what's covered and what isn't. **The full browser matrix must run against the bundled `dist`, not the raw `site/`** — `npm test`, `npm run test:all`, and `npm run test:dist` all build first; `npm run test:browser` runs against the unbundled `site/`, which flakes on webkit under the cold-load module waterfall, so use it only for single-browser chromium iteration. **Don't re-run the suite just to recover results you already have** — the last run is saved under `test-results/` (`.last-run.json` for status + failed-test IDs, one folder per failed test). Read those instead of re-running, and prefer targeted runs (`npm run test:browser -- <file> --project=chromium`) over the full matrix when verifying a fix. **Browser runs serialize repo-wide** through [`scripts/with-test-lock.sh`](scripts/with-test-lock.sh), so only one worktree runs the suite at a time: `another worktree is running tests, waiting for the lock...` is expected and can last minutes, not a hang — don't kill it and don't route around it. A bare `npx playwright test` bypasses the lock; the `npm run test:browser -- …` form takes it.
...
- [`docs/components.md`](docs/components.md) — **single source of truth for the shared UI vocabulary**: every export of `ui/components.js`, the cross-cutting helpers (dialogs, icons, toasts, score badges, text), and the CSS classes they emit. Read before adding any component, builder, or CSS class — it exists so existing pieces get reused instead of rebuilt.
...
- [`docs/worker-protocol.md`](docs/worker-protocol.md) — the main↔pipeline-worker contract: data ownership, the message protocol, cancellation/supersession. Read before touching the worker boundary; keep it current in the same commit as any protocol change. The streaming invariant it enforces has its own section, *Streaming is authoritative*, below.
...
When a plan ships, run the `distill-design-doc` skill to fold it into `design.md` and/or `manual.md`. (The header `?` button opens the in-app Help dialog (`HelpDialog`) — an FAQ with a folded-in Acknowledgements section, deep-linkable at `#/help`, that renders its diagrams and credits from the live catalog.)
...
## Streaming is authoritative — no settling

When a run streams, the streamed snapshots *are* the result. The terminal `result` does end-of-stream **bookkeeping only** — flip out of the busy state, ship the final window, adopt the streamed order/histogram/stats — and must **never** recompute, re-sort, re-filter, or finalize a streamed result. A terminal recompute runs a different code path than streaming, so it silently overwrites (masks) any streaming bug instead of surfacing it. A full compute at completion is legitimate **only** on the non-streamed path — a result small enough it never streamed. Details in [`docs/worker-protocol.md`](docs/worker-protocol.md).
...
- `type` — `'edits'` for My Edits; absent for all regular sources. Test it with `wordlist.type === 'edits'`, never against a string-constant ID.
...
**Publishers** (`WORDLIST_PUBLISHERS`) — the known wordlists (JK, XWI, Nediger, STWL, Broda) are config bundles, not identities. Each carries an `id`, a fetch `url`, `defaultRules`, and display fields (`name`, `author`, `icon`, `homepage`, `popularity`, the `sourcePage`/`sourceNote`/`subscriptionNote` blurbs). `getPublisher(wordlist)` looks up by `wordlist.publisherId`, and that reference is the only way to resolve one — nothing infers a publisher from a wordlist's key, name, or filename.
...
**Disk sync is a per-list layer on IDB, not a backend.** IDB is always canonical; `Storage` is the sole storage object — one venue, no backend abstraction to dispatch through. A list optionally syncs to a file: My Edits is bidirectional (watched + 3-way merged against a baseline), every other list is a one-way output mirror. See [`docs/design.md`](docs/design.md) § *Disk sync*.
...
**One spelling per norm** — a norm can carry several spellings (`eta`/`ETA`, `theirs`/`the IRS`), each with its own score and comment. `norms` (a `Set` on the corpus) answers existence only; anywhere one spelling must stand for all of them, call `bestRowForNorm(merged, norm)` — highest score, then the shorter spelling, then code-unit order. Never re-derive that pick at a call site, and never reach for a row when membership is the question.

**Virtual scroller** — `BaseVirtualScroller` renders only visible rows (`EntriesScroller` extends it). Row height is fixed.
...
- **Reuse before you build.** Before adding a component or a CSS class, grep `site/src/ui/` and `site/css/` for the name you're about to give it — not for what it will look like — and read the whole module you're extending, not just the function. A related-but-different hit is not a negative result; keep looking. The shared vocabulary is catalogued in [`docs/components.md`](docs/components.md).
...
- **"Download" means output only.** Use "download" exclusively for saving a processed wordlist from Grawlix to disk (`downloadMergedWordlistFromPanel`, `downloadSourceWordlist`, `downloadOriginalWordlist`). Use "fetch" for getting a wordlist into Grawlix from a URL (`fetchWordlist`), and "import" for the user loading a file. Template properties that refer to a third-party source page use `sourcePage` / `sourceNote`.
...
Generic builders live in `ui/components.js`; domain builders live in their consumer module. Nothing outside a component should reach into its DOM subtree. [`docs/components.md`](docs/components.md) catalogues what `ui/components.js` already provides.
