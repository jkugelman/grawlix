...
- [`docs/planned/result-cache-followons.md`](docs/planned/result-cache-followons.md) — three forward-looking extensions of the shipped in-memory result cache: partial-run retention (re-run + catch-up), intermediate-step reuse (generalizing `_preSearchCache` into a keyed prefix cache), and the parked coroutine-resume idea. All reuse the shipped cache's substrate (keyed map, byte budget, corpus-identity invalidation).
