...
- [`docs/worker-protocol.md`](docs/worker-protocol.md) — the main↔pipeline-worker contract: data ownership, the message protocol, cancellation/supersession. Read before touching the worker boundary; keep it current in the same commit as any protocol change. **Streaming is authoritative — no settling.** When a run streams, the streamed snapshots *are* the result; the terminal `result` does end-of-stream **bookkeeping only** (flip out of the busy state, ship the final window, adopt the streamed order/histogram/stats) and must **never** recompute, re-sort, re-filter, or finalize a streamed result. A terminal recompute runs a different code path than streaming, so it silently overwrites (masks) any streaming bug instead of surfacing it. A full compute at completion is legitimate **only** on the non-streamed path (a result small enough it never streamed). See the principle in `worker-protocol.md`.
