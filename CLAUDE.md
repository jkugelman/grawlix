...
- [`docs/planned/result-cache-followons.md`](docs/planned/result-cache-followons.md) — the parked forward-looking extensions of the shipped in-memory pipeline caches: coroutine resumption (resume an abandoned run *without* the catch-up redo) and an IndexedDB L2 for reload-survival. Both would reuse the shipped `GdsCache` substrate (keyed map, byte budget, corpus-identity invalidation). Two ideas the doc once tracked have **shipped** and are now in `design.md` § *Streaming results*: intermediate-step reuse as the `prefixCache` (a keyed prefix-state cache — every stage boundary a uniform tile, no privileged slot) and partial-run retention as the `partialCache` (a superseded streaming run stashes its incomplete join; re-entering resumes it — flat/tuple cursor-validate, transform rebuilds-and-swaps).
- [`docs/planned/editing.md`](docs/planned/editing.md) — entries-table editing plan. Phase 1 (flat tier): a data-tracked selection (virtual-scroller-safe `role="listbox"` + `aria-activedescendant`), arrow/PgUp/Home nav, click-to-select, contiguous + non-contiguous multi-select, Enter-to-open with focus-return, batch Alt+# rescore / Delete as one undoable write-set, and hover-as-target removed in favor of selection. Selection is keyed on the *atom* (a flat row is the one-atom case) so the multi-atom transform/group tiers plus a multi-entry panel editor extend additively (parked, open questions recorded). §9 is the undesigned **bulk-editing stretch goal** with the motivating real-world cases captured verbatim (spacing out unspaced families, adding punctuation, conjugation-matched comments, name-variant comments).
