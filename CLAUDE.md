...
`state` holds `sources` (the per-wordlist data), `scoring` (tier labels for the unified score scale, used everywhere scores are displayed — the merged All view shows them as a legend), `scoringDirty` (true when tier labels diverge from `DEFAULT_SCORING`), and search state. Each wordlist has metadata, `rawEntries` (parsed wordlist-entry records, shape `{ entry, score, comment }`), `rescoreRules` (always `[]` for My Edits), and a `dirty` flag against its publisher's `defaultRules` (`getWordlistDefaultRules` returns null for My Edits, so propagation skips it). Transient `wordlist._uncovered` / `state._scoringUncovered` carry the list of scores not covered by any rule — see *Rescore rules & tier alignment* in `docs/design.md`.
...
**My Edits** — a special wordlist created automatically on first boot, identified by `wordlist.type === 'edits'`. No rescore rules: its `rescoreRules` is held at `[]`, the Library panel suppresses the rescore editor / Rescored-Original toggle, and `recomputeUncovered` short-circuits to `_uncovered = []` for it. User-typed scores are already on the unified scale by construction, so there's nothing to map and the misalignment bubble never trips on My Edits' scores regardless of tier labels. In the rules-editor slot My Edits renders the scoring (tier-label) editor — same `state.scoring` data and same `#wld-scoring-section` element id as All's panel, so `refreshScoringSection` updates whichever panel is focused. Clicking a score or comment cell in the Workshop opens an inline editor (`AtomPopover`); saving routes the edit into My Edits regardless of which wordlist sourced the row. From the My Edits view the user can also add new entries and delete entries (with undo). It is reorderable like any other wordlist (position determines merge priority). The UI enforces: not deletable, always enabled.

**Merged wordlist** — `MERGED_ID = '__merged__'` selects a union of all enabled sources, deduped by entry. The highest rescored value wins; losers are shown faded with a tooltip. Displayed as `All` (the value of `MERGED_NAME`) at the top of the wordlist card list in the left rail's Wordlist section. `buildMergedWordlist` resolves the winner per `(norm, display)` directly while bucketing all enabled sources' rescored entries (highest-priority eligible contributor wins); the Workshop scroller only ever receives the already-resolved merged entries, so it has no raw-vs-effective distinction of its own. My Edits edits patch the affected norms into the cached merged result in place via `applyEditsChange` / `patchMergedForNorms` rather than rebuilding it.
