...
- [`docs/plans/tools.md`](docs/plans/tools.md) — remainder of the catalog, indexed-view runtime, async/cancellation/spinners, highlights & annotations, groups output, gallery polish (category picker, search), result download, OneLook/Datamuse/Umiaq. Pipeline runtime + pair output + per-kind sort shipped (see `design.md`).
