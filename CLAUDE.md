...
- [`docs/planned/result-cache-followons.md`](docs/planned/result-cache-followons.md) — two forward-looking extensions of the shipped in-memory pipeline caches: partial-run retention (re-run + catch-up) and the parked coroutine-resume idea. Both reuse the shipped `GdsCache` substrate (keyed map, byte budget, corpus-identity invalidation). The doc's third idea, intermediate-step reuse, has **shipped** as the `prefixCache` (a keyed prefix-state cache that *subsumes* the old pre-search cache — every stage boundary is a uniform tile, no privileged slot; see `design.md` § *Streaming results*).
