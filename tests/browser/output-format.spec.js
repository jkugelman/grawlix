...
test.describe('output format setting', () => {
  test('getOutputFormat defaults to fully rich and round-trips through mergedSettings', async ({ page }) => {
    ...
    expect(await page.evaluate(() => getOutputFormat())).toMatchObject(
      { spaces: true, punctuation: true, digits: true, diacritics: true, symbols: true, comments: true });

    const got = await page.evaluate(() => {
      const stripped = { spaces: false, punctuation: false, digits: true, diacritics: false, symbols: true, comments: false };
      Storage.writeMergedSettings({ ...Storage.readMergedSettings(), outputFormat: stripped });
      return getOutputFormat();
    });
    expect(got).toMatchObject({ spaces: false, punctuation: false, digits: true, diacritics: false, symbols: true, comments: false });
  });
});

test.describe('output format UI', () => {
  test('Settings exposes the six flag checkboxes and no case toggle', async ({ page }) => {
    await gotoApp(page);
    await page.evaluate(() => SettingsDialog.open());
    await expect(page.locator('#output-format-ctrls .of-flag')).toHaveCount(6);
  ...
  test('changing a flag in Settings persists immediately, before closing', async ({ page }) => {
    await gotoApp(page);
    await page.evaluate(() => SettingsDialog.open());
    await page.locator('#output-format-ctrls input[data-flag="diacritics"]').uncheck();
    expect(await page.evaluate(() => getOutputFormat().diacritics)).toBe(false);
  });
  ...
  test('a source downloads immediately, applying the global output format with no dialog', async ({ page }) => {
    ...
    await page.evaluate(() => setOutputFormat({ spaces: false, punctuation: false, digits: true, diacritics: false, symbols: true, comments: true }));
...
test.describe('results exports follow the output format', () => {
  ...
  const STRIP = { spaces: false, punctuation: false, digits: true, diacritics: false, symbols: true, comments: true };
  ...
  test('Results as wordlist drops comments when the comments flag is off', async ({ page }) => {
    await gotoApp(page);
    await addRichFixture(page);
    await page.evaluate(() => setOutputFormat({ spaces: true, punctuation: true, digits: true, diacritics: true, symbols: true, comments: false }));
  ...
  test('with digits off, Results as wordlist and CSV leave out entries with digits whole', async ({ page }) => {
    await gotoApp(page);
    await page.evaluate(() => window.__grawlixTest.addCustomWordlist({
      name: 'Nums', entries: ['R2D2', 'BLUE JAY'], scores: [50, 50],
    }));
    await page.evaluate(() => setOutputFormat({ ...getOutputFormat(), digits: false }));

    const { text } = await page.evaluate(() => window.__grawlixTest.exportText('wordlist'));
    expect(text).toContain('BLUE JAY;50');
    expect(text).not.toMatch(/R2D2|RD;/);

    const csv = await page.evaluate(() => window.__grawlixTest.exportText('csv'));
    expect(csv).toContain('BLUE JAY,');
    expect(csv).not.toMatch(/R2D2|RD,/);
  });

  test('with symbols off, Results as wordlist and CSV leave out entries with symbols whole', async ({ page }) => {
    await gotoApp(page);
    await page.evaluate(() => window.__grawlixTest.addCustomWordlist({
      name: 'Syms', entries: ['A+', 'AT&T', 'BLUE JAY'], scores: [50, 50, 50],
    }));
    await page.evaluate(() => setOutputFormat({ ...getOutputFormat(), symbols: false }));

    const { text, excluded } = await page.evaluate(() => window.__grawlixTest.exportText('wordlist'));
    expect(text).toContain('BLUE JAY;50');
    expect(text).not.toMatch(/A\+|AT&?T/);
    expect(excluded).toBe(2);

    const csv = await page.evaluate(() => window.__grawlixTest.exportText('csv'));
    expect(csv).toContain('BLUE JAY,');
    expect(csv).not.toMatch(/A\+|AT&?T/);
  });
