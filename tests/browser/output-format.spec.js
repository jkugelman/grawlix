...
test.describe('output format setting', () => {
  test('getOutputFormat defaults to fully rich and round-trips through mergedSettings', async ({ page }) => {
    ...
    const got = await page.evaluate(() => {
      const stripped = { spaces: false, punctuation: false, digits: true, diacritics: false, unicode: true, comments: false };
      Storage.writeMergedSettings({ ...Storage.readMergedSettings(), outputFormat: stripped });
      return getOutputFormat();
    });
    expect(got).toMatchObject({ spaces: false, punctuation: false, digits: true, diacritics: false, unicode: true, comments: false });
  });
});
...
test.describe('output format UI', () => {
  ...
  test('every flag is labeled with what unchecking it strips', async ({ page }) => {
    ...
    await expect(page.locator('#output-format-ctrls .of-flag')).toHaveText(
      ['Spaces', 'Digits', 'Punctuation', 'Diacritics', 'Unicode', 'Comments']);
  });
  ...
  test('a source downloads immediately, applying the global output format with no dialog', async ({ page }) => {
    ...
    await page.evaluate(() => setOutputFormat({ spaces: false, punctuation: false, digits: true, diacritics: false, unicode: true, comments: true }));
...
test.describe('results exports follow the output format', () => {
  ...
  const STRIP = { spaces: false, punctuation: false, digits: true, diacritics: false, unicode: true, comments: true };
  ...
  test('Results as wordlist drops comments when the comments flag is off', async ({ page }) => {
    await gotoApp(page);
    await addRichFixture(page);
    await page.evaluate(() => setOutputFormat({ spaces: true, punctuation: true, digits: true, diacritics: true, unicode: true, comments: false }));
