...
// the whole bar — count and histogram — tracks the streamed-so-far set rather than
// going stale or waiting for completion. The terminal result clears all of it.
//
...
test('the panel marks streaming and the stats bar tracks the streamed-so-far set, then the terminal result clears it', async ({ page }) => {
  ...
  // The histogram stays visible: the overflow logic never hides it on a normal
  // viewport, so the bar must NOT carry stats-no-hist (regression guard for the
  // dots disrupting the stats-bar grid).
  ...
  // A score-69 bucket filled in live: the histogram tracks the streamed-so-far
  // set, not a stale or final-only distribution.
  expect(result.b2.histBars).toBeGreaterThan(result.b1.histBars);
