...
test('edit — rescored two-norm move hands the winner to a rescored source', async ({ page }) => {
  await gotoApp(page);
  await seed(page);
  // Alpha rescores CRANE (60 → 85), so once My Edits vacates "crane" Alpha's
  // rescored row (rawScore 60) wins the merged bucket.
  await page.evaluate(() => window.__grawlixTest.setRescoreRules('Alpha', [
    { input: '60', length: '', output: '85', note: '' },
  ]));
  // My Edits owns "crane" too (highest priority → wins until the move).
  ...
  assertConverged(r);
  // "crane" handed off to Alpha (rescored), "cranex" is the new My Edits entry.
  ...
  expect(crane[2]).toBe(85);                          // rescored effective score
