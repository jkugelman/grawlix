...
test.describe('Acronyms tool', () => {
  ...
  async function setAcronym(page, q) {
    await page.evaluate(target => window.__grawlixTest.setStack([
      { tool: 'acronyms', params: { acronym: target } },
    ]), q);
  }
...
test.describe('Acronyms grouped mode', () => {
  async function setGrouped(page) {
    await page.evaluate(() => window.__grawlixTest.setStack([
      { tool: 'acronyms', grouped: true },
    ]));
  }

  test('clusters multi-word entries by their acronym when the acronym is an entry', async ({ page }) => {
    await gotoApp(page);
  ...
  test('drops clusters whose acronym is not an entry in the wordlist', async ({ page }) => {
    await gotoApp(page);
  ...
  test('skips single-word entries (one-letter acronyms are just prefix buckets)', async ({ page }) => {
    await gotoApp(page);
