...
Not a search-only slice. Since the pipeline core is already its own `engine/` layer (next section), running only search in the worker would leave most of that code unused on the main thread and create a split-brain pipeline — transforms on main, search in the worker, with a handoff between them. So the entire executor — `executePipeline`, `runToolStage`, `bucketize`, `unify`, every tool's `run` / `prepare` / `group`, the cooperative yielder — lives behind the boundary. The main thread, in the steady state, runs no pipeline code at all.
...
### The worker imports `engine/`

The executor and everything it transitively calls is already a **self-contained, DOM-free island** — that's exactly what the `engine/` layer is ([`design.md` § Code structure](../design.md#code-structure)). The tools, `toNorm`, `buildSearchPattern`, the phrase segmenter, the unigram corpus: none touches `document`, `window`, `localStorage`, or `navigator`. So the worker doesn't need any source-extraction plumbing — it loads as a module worker (`new Worker(url, { type: 'module' })`) and `import`s `engine/` directly, running the *identical* module text the main thread runs, so a tool's `run` can't drift between threads.

The boundary is already drawn cleanly by the layering, and two facts of it carry straight over:

- **The unigram corpus loader takes injected I/O.** `engine/segmenter.js`'s `loadUnigramCorpus` (Space out's `prepare` dependency) does `fetch` + IDB caching (both worker-safe) and a `localStorage` size note (not worker-safe), but it never imports `data/`: I/O arrives via `configureSegmenterIO({ idbGet, idbPut, onSize })`. The main thread injects real IDB + an `onSize` that writes the LS size note; the worker host injects worker-native IDB + a no-op `onSize`. (The worker-era alternative — build the frequency map on main and ship it — is still open; see [`design.md` § Code structure → Open questions](../design.md#code-structure).)
- `compileRescoreRules` and the rescore layer are **not** in `engine/` — they're `data/`, below merge, and run *before* the merge. The worker receives entries already rescored and already merged; it never rescores.

The build is ready for this: `build.mjs` is structured so `engine/worker.js` becomes a second esbuild entry point, emitted as its own bundle (a worker can't share the main bundle's scope).

> A deployment note: a module worker is served from a same-origin URL — no Blob URL, so the `worker-src blob:` CSP caveat doesn't apply. `SharedArrayBuffer` is off the table regardless (it needs COOP/COEP cross-origin-isolation headers GitHub Pages can't set); the design does not need it.
...
- **The `engine/` layer must stay DOM-free forever.** A test should boot the worker and assert it doesn't throw, so an accidental `document.…`/`localStorage.…` slipping into `engine/` is caught in CI rather than as a silent worker-boot failure in the field. (The injected-I/O segmenter loader above is the pattern that keeps the one real I/O violation out of the engine.)
...
- **Browsers without workers** are not a concern (workers are universal), but the fallback path — running the executor on the main thread, which it already can since the main thread imports `engine/` too — is worth keeping wired as a safety net and for worker-spawn/crash recovery. The cost is that `_preSearchCache` then exists in two venues; the fallback must own cache coherence when it takes over.
