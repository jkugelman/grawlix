...
- **Per-tool layout — generic renderer vs. arbitrary row HTML.** `buildParamsHTML` / `buildParamHTML` render every tool's stack row from its `params` schema as one flat flex row — this is what drives consistency and the single-catalog principle (a tool is one `TOOLS` entry; the search bar was *unified into* this renderer, having previously had its own builder). But the renderer is layout-rigid. Regex is the first tool to strain it: two text inputs that read as a find/replace box — pattern stacked over replace, with the icon/label/whole-word centered on the first line — can't be expressed cleanly, and coercing it into the flat row invites workarounds (fixed-height spacer containers and the like). The escape hatch would be letting a tool supply its own row HTML — the parked `renderParams` field (see *Escape hatches* above). The catch: arbitrary HTML doesn't free a tool from the machinery. The `params` schema is still read by URL encode/decode, `PARAM_HELP`, `isInert`, and the executor, and every input still needs `data-row`/`data-key` for the delegated state handler — so a custom-HTML tool is "own layout *plus* re-wire the plumbing by hand," and each tool's layout then drifts independently, eroding the consistency the unification just bought. The narrower fix is to make the generic renderer *layout-aware* — choose a layout from param shape (e.g. 2+ text inputs → a two-row grid), still fully data-driven, no per-tool HTML. Default to that while Regex is the lone outlier. Revisit the `renderParams` escape hatch if, as the catalog grows to dozens of tools, custom or mobile-awkward layouts become common rather than exceptional.
