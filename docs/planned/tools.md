...
- **Synthetic-atom downstream behavior.** What does `[phrase_parsing, head_off]` mean? The synthetic "hot to trot" goes into head_off, which tries to look up "ot to trot" in `wordlist.byEntry`, finds nothing, drops the row. Probably degenerates harmlessly but the chained semantic is fuzzy. Rebus (shipped) is now a synthetic emitter too — `[rebus, head_off]` beheads a glyph form that exists in no wordlist, so it drops the same way. Revisit if a real workflow surfaces.
