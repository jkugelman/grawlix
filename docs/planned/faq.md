...
27. **How do I find phrases hiding a word in their initials?** — *Initialisms all-mode: "then it’s settled", "this is serious", and the run-together "timeisshort" all land under TIS — and only surface if TIS is itself an entry (bidirectional pairs). Great theme generator.* **[★]**
