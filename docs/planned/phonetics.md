...
- `rhymingPartsOf(text, mode)` looks up the **last word** of a phrase — a phrase rhymes on its last word only. An entry with no spaces that CMU doesn't know is spaced out first (`bestSpaceOutSplit`, the Space out segmenter), which is what lets an unspaced wordlist rhyme at all: measured against XWI, Spread the Wordlist and Broda, the share of entries that can rhyme goes from 13–22% to 75–80%. The dictionary wins where it has the whole string (`NOTABLE` is not `NO TABLE`), and a one-letter final part is rejected as `rankedSplits`' short-part escape hatch (`YOWLERS → YOWLER S`). Where the segmenter declines outright — the corpus carries the glued form as a word of its own, so the unsplit entry wins its own score window (`RICKROLL` beats `RICK ROLL` by 10.04 against a window of 10) — `bestCompoundSplit` supplies a second answer: the best split into 2+ parts with the unsplit form excluded, gated on every part being a common wordlist entry and the last running 3+ letters, and peeling an inflection first so `RICKROLLING` reads as `RICK ROLLING` while `CALLUSING` stays whole. Space out gets no such fallback, `RICKROLL` being legitimately one word there; here an entry that cannot be read rhymes with nothing at all. Worth 9,677 more readable entries over a 738k merge, with none lost. Details in `design.md` § *Space out*.
...
Six rules carry it, each validated against CMU and the shipped wordlists:
...
- **The cot–caught merger, but never before R.** CMU records the AO/AA split, and records it arbitrarily: CAUGHT is listed both ways, GONE only ever AO, DON only ever AA, ON both — 10,164 of the 10,574 AO headwords carry no AA variant, so the exceptions are the coin flips. Honoring it tears one family in half along one of those flips: GONE PRO and COINTREAU bucketed away from DON HO, JOHN DOE and TONTO, with ON AUX, ON LOW and ON TOE stranded in each half, which reads as a duplicate-row bug rather than a pronunciation one. Folding the pair fixes that — except before R, where no dialect merges NORTH with START and folding rhymes CARD with CORD, BARB with ORB, CARD PLAYERS with BOARD GAMERS. The R has to be one the vowel's own syllable closes on, since a word-initial R colors nothing before it: SAW RED folds, SORE ED does not. This is the one rule here that also governs Loose and Strict, where COT and CAUGHT had likewise not rhymed. On a 738k-entry merge it takes Whole from 46,520 families to 46,685 and Loose from 7,173 to 7,128 — it fuses families, and more often forms new ones out of AO-only words that had no partner at all. (CMU also lists six headwords both R-ful and R-less, CARS and FOR among them, which is why OLD CARS reaches OLD SAWS. Six in 122,431: a dictionary wart, left alone.)
