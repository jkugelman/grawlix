...
Rescore-rule authoring lives in an **inline-expandable area** on the wordlist bar, default-collapsed, summoned when you want it. Its trigger is a compact **adjustments (sliders) icon** just right of the selector — in the bar's left, view-defining cluster (selector + this), not among the per-source action buttons on the right, because rescoring shapes *how the scope is presented* rather than acting *on* the list. An icon (not a word) keeps it quiet beside the selector-as-title and sidesteps the trigger's polymorphism: it carries a tooltip ("Rescoring rules" on a source, "Scoring tiers" on All) instead of swapping its label. Opening and closing the area is animated. It is **usable while scrolling** — pinned in place so the entries table scrolls beneath it — because the whole point is to tune a rule and watch its effect.
