...
A **manage panel** owns the *cross-list* operations: reorder, enable/disable, add wordlist. It is reached from **inside the selector dropdown** — a "Configure wordlists" action in a divided footer below the scope rows — because the picker and the manager operate on the same object (the set of wordlists): the picker is the read view (scope to one), the manage panel its edit view. This keeps the bar calm and reserves its right edge for per-source actions; the slot beside the selector that an always-visible adjustments button would have taken instead hosts the rescore trigger (see [Rescore and scoring](#rescore-and-scoring)). The trade is discoverability — a dropdown footer advertises less than a visible button — accepted because list management is infrequent and the selector is exactly where one goes when thinking about wordlists. It is deliberately separate from per-source configuration (which stays per-source — see below), because reorder and enable are cross-list concerns while rescore, rename, and the like are properties of one list.
...
- **The sync sign** stays exactly as it is today — a small dot-plus-status card (the Library's `.wld-sync-hang` / `.wld-sync-sign`) that **hangs below the bar on the right, beneath the Download button** (its right edge aligned to the tool-gallery's content inset), the single always-present element for disk-sync status and control. It hangs *off* the bar rather than sitting in the action row, and is never folded into a menu.
- **The split Download button** — Download (rescored output) / Download original — the curation payoff, with a tooltip naming what it saves.
- **A slim per-source kebab `⋮`** — the rest: **Fetch/Import**, **Configure**, **Delete**. Configure is the existing dialog and already holds rename, change icon, publisher binding, the auto-update URL, an import section, and the rules-option — so rename/icon need no standalone entries. The F2-on-a-card rename shortcut is dropped (cards now live inside the picker and manage panel). "Apply rescoring permanently" (bake) does **not** live here — it moves into the rescoring panel (below), beside "Disable rescoring", since both are rescoring-lifecycle actions.

So the right-hand action cluster is **Download + the adjustments (rescore) toggle + ⋮**; on **All** it's **Download + adjustments** (the merged product, no kebab). The bar's left is just the selector (the title). Responsive behavior **reuses the Library title bar's** (don't reinvent): the selector label ellipsis-truncates, the **"Last updated" label hides at ≤759px** (the existing `.detail-date` rule), and the cluster `flex-wrap`s under extreme pressure — no bespoke fold-into-kebab.

This is three menu-ish surfaces in total — the cross-list manage panel (in the selector dropdown), the per-source kebab (fetch/configure/delete *this list*), and the **export kebab in the stats bar** (copy / download-as-wordlist / CSV / JSON of *the current filtered/sorted view*). The bar's **Download** and the stats-bar export overlap only in appearance: Download saves **the whole wordlist** (the list as an asset), the stats-bar kebab exports **what's currently shown** (`workshopScroller.entries`, the post-pipeline result set). The fix is **sharper labels by scope**, not removal — the bar says *Download* (the list); the stats-bar menu is relabeled to read as *export these results* (the view). Each stays where it's contextually right (list-output by the list identity, results-export by the results table).

## Rescore and scoring

### One inline editor, polymorphic by scope

Rescore-rule authoring lives in an **inline-expandable area** on the wordlist bar, default-collapsed, summoned when you want it. Its trigger is an **adjustments (sliders) icon button** in the bar's right-hand action cluster, **directly left of the kebab** — the bar's left is just the selector (which doubles as the title), and all controls (Download, this, the kebab) sit together on the right. An icon (not a word) sidesteps the trigger's polymorphism: it carries a tooltip ("Rescoring rules" on a source, "Scoring tiers" on All) instead of swapping its label. It's styled like the other bar buttons (border + background), with an accent fill when open; opening and closing the area is animated. It is **usable while scrolling** — pinned in place so the entries table scrolls beneath it — because the whole point is to tune a rule and watch its effect.

And the effect shows up in **the main table itself**: when the rescore editor is open, the scoped source's rows display their raw→rescored `→` annotations (the same authoring feedback the Library shows today), so the table *is* the live preview. That collapses the old Library rescore section, which had to carry its own preview list, down to just the rule rows plus Add / Reset / Disable rescoring / Apply rescoring permanently (bake). Default-collapsed, it costs nothing until summoned.
...
- **Backdrop-dismiss is an app-wide change, not a panel detail.** *Resolved* — `createDialog` gained a `dismissOnBackdrop` flag (default true); off for dialogs holding unsaved edits. Landed as its own change (configure-wordlist flipped; the manage panel will too).
- **Mobile wordlist-bar overflow.** *Resolved* — reuses the Library title bar's responsive behavior (ellipsis-truncate the selector, hide "Last updated" ≤759px, `flex-wrap` the action cluster, sync sign hanging beneath the selector). With manage moved into the dropdown, the rescore trigger a single icon, and only Download + ⋮ on the bar, it's far lighter than first feared — no bespoke collapse. See *Per-source actions*.
