...
A scoped source is a small merge — the source plus the user's My Edits overlay (see [The editing model](#the-editing-model-my-edits-as-a-universal-overlay)) — feeding the same `executePipeline` the merged view uses. The pipeline *logic* is reused, but building a scoped corpus is **net-new code**, not a free consequence of an existing parameter. `buildMergedWordlist` today takes no arguments and memoizes one global cache, and the single-source path is a flat `getRescoredEntries(...)` array with no `byNorm` / `byKey` / `sourceCounts` / `_initialChains`. A scoped corpus must synthesize all of those, invalidate the module-global `_preSearchCache` on every scope switch, and make the histogram layout (today computed over the union of *all* sources) scope-aware. The source column, meaningless when one source fills every row, hides under scope.
...
"The selection is the corpus" deliberately means **scope** (the corpus *becomes* that source, plus your personal My Edits overlay), not **filter** (subset the merged view). With sources in priority order **My Edits > Broda > XWI**, and `ocean` at Broda 90 / XWI 70, `tide` at XWI 40 with your My Edits edit of 55, and `zebra` only in Broda at 60:

| View | `ocean` | `tide` | `zebra` |
|---|---|---|---|
| **All** | 90 (Broda wins) | 55 (My Edits) | 60 (Broda) |
| **Scope = XWI** | 70 (XWI's own — Broda not mixed in) | `40 → 55*` (your override on XWI's 40) | *absent* (not in XWI) |
...
Two arrow annotations share this `old → new` vocabulary, distinguished by a trailing asterisk: a **rule** arrow `350 → 80` (no asterisk), shown on rule-changed rows while the rescore editor is open, and a **user-override** arrow `70 → 75*` (asterisk), shown in scoped-source views wherever your My Edits overlay supersedes the base score (see [The editing model](#the-editing-model-my-edits-as-a-universal-overlay)). The asterisk reads as *this change is yours, not a rule's*.
...
## The editing model: My Edits as a universal overlay

Edits always save to My Edits — that rule is unchanged. The redesign settles how that interacts with scope: **My Edits is a universal personal overlay, present in every scope.** Scoping to a source shows that source's entries *with your My Edits overrides applied* (resolved by the normal priority order). My Edits is *you*, not another publisher, so it follows you everywhere; only other publishers' lists are excluded by scope.

This resolves what would otherwise be a contradiction. Without the overlay, "scope = the source by itself" collides with "edits go to My Edits": you'd edit `ocean` while scoped to XWI, the edit would land in My Edits, and the XWI-only table would keep showing XWI's old value — the edit would appear to vanish. With My Edits as the overlay, the edit shows up immediately, exactly as it does on All today, governed by the same My-Edits priority. (This also means the edit-patch path — `applyEditsChange` / `patchMergedForNorms`, which today patches the one global `_mergedWordlistCache` in place — must target whatever the *active* corpus is, scoped or merged.)

**The override indicator.** Where your overlay supersedes the base source, the score renders as **`base → edited*`** — the arrow shows old→new inline, the trailing `*` marks it as a *user edit* (versus a rescore rule's plain `350 → 80`). Both sides are *effective* (rescored) values — the base source's rescored contribution and My Edits' rescored contribution — so they're comparable on the unified scale. The edited value shown is the **rescored** one, not the raw typed number, consistent with the table always showing effective scores; in the common case (My Edits ships a pass-through legend) the two are equal, so it reads as you'd expect. The indicator is **scoped-only by nature**: the `base →` half needs a single base to point from, which exists only when scoped to one source — All has no single base, so it keeps the source column and the popover instead. (The red-asterisk override marker that design.md's identity-contrast table still lists is *not in the current code*, where the Workshop score atom is bare; this re-adds it, in arrow form.)

**The popover as a provenance panel.** Scoping still raises a hazard — you might edit a value a higher-priority list supersedes — so the `AtomPopover` becomes an editor *and* a full-provenance info panel. Regardless of scope it shows the complete merge picture for the entry: every contributing wordlist, in priority order, with its display variant, effective score, and comment, flagging which wins each field. This (a) **makes scoped editing safe** — the popover is the cross-wordlist truth even when the table is scoped, so you never edit a superseded value blind; (b) **fixes the "lying Source column"** — the table attributes one source per row, but display, score, and comment can each come from a *different* contributor (comment fall-through is designed to do this — see [`../design.md`](../design.md#rich-wordlists)); and (c) **serves the trust use case** — a constructor who doesn't auto-believe the top-priority list can see at a glance whether others scored an entry higher.

The provenance rows show **effective (rescored) scores only** — no per-wordlist `raw → rescored`. The popover answers the trust question, and comparable rescored scores serve it; raw scores are per-wordlist-scale and non-comparable, so a raw→rescored column per contributor is overload (raw→rescored belongs in the rescore editor).

The contributor *lookup* is cheap — `wordlist._rescoredByNorm` already exists per source, so gathering contributors for one norm is O(sources) — but the **panel itself is a new sub-component**: today the popover shows a single winner-source row, so the multi-contributor, per-field-winner, variant-aware layout is net-new. And it must include **disabled and non-winning** contributors (the trust case is precisely "did a list I'm *not* currently merging score this higher?"), so it cannot reuse `computeMergedBucket`'s enabled-only walk.
...
- **The editable score field: raw or rescored?** The popover's score *input* currently shows the effective (rescored) score, not the raw typed value. Once My Edits carries non-pass-through rescore rules, we must decide whether the edit field shows and edits the *raw typed* value (what gets stored) or the *rescored* value (what's displayed) — saving one as the other is lossy. Invisible today because My Edits usually ships a pass-through legend (raw == rescored), but scoped editing makes it prominent enough to settle deliberately.
- **Provenance popover presentation.** How the panel handles `(norm, display)` variants (a norm can carry several display rows — `theirs` vs `the IRS`) and ambient null-display contributors (which feed every variant of a norm); the layout of the multi-contributor list. The mechanism is settled (O(sources) over `_rescoredByNorm`, including disabled/non-winning); the presentation isn't designed yet.
