...
**The yield must be a macrotask** (`setTimeout(0)` or a `MessageChannel` self-ping). A *microtask* yield (`queueMicrotask`, resolved promise) drains only microtasks and **never** delivers the `message` task. `scheduler.yield()` is to be **avoided** in the worker: its boosted-priority continuation may starve the `message` event (Chromium-implementation-dependent, not a spec guarantee). The macrotask is the only yield the event-loop model *guarantees* delivers the message. (Verify empirically before building on it — the B1 spike.)

**A superseded run sends NO reply** (it throws `AbortError` internally and moves on). The main client therefore **must settle that `runId`'s pending promise itself** (as aborted) — otherwise `pipelineIdle()` (which the whole Playwright suite awaits after a keystroke) dangles and every post-keystroke assertion times out, suite-wide. Track runs so an abandoned `runId` resolves rather than hangs.
