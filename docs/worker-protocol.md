...
#### `fetchRows` ✅ (windowed row fetch, interim row shape)
`{ type: 'fetchRows', requestId, runId, start, end }` — **request a window `[start, end)` of the retained flat result.** The async-scroller stage ([`planned/worker-data-tier.md`](planned/worker-data-tier.md) § *Staging*, stage 2). The windowed flat scroller is now the **default** render path (`WINDOWED_FLAT`): main fetches its visible window + prefetch buffer, renders shimmer skeletons on a miss, and serves ordinary scrolling from a bounded local cache. A score-range filter falls back to the local render (filtered positions don't align with the worker's result array — a stage-2 gap closed in stage 3).
...
- `runId` — which flat result this window names. The worker retains the **last flat result** only (`lastFlatResult = { runId, indices, scores }`, the sorted survivor indices `.slice()`d before the `run`'s transfer detaches the originals — main still receives the full arrays unchanged). A grouped or rich (transform-chain) result nulls `lastFlatResult`, so a fetch for a non-flat result is dropped by the runId guard.
- `start` / `end` — the half-open window into the sorted survivor list. `start` is floored at 0, `end` capped at `length`; a `start` past the end simply yields an empty `rows` (the loop runs zero times). `start | 0` / `end | 0` coerce.
- **On receipt:** if there's no retained flat result or `runId` doesn't match it, **drop silently** (no reply) — the same supersession discipline as runs; main re-fetches against the current result. Otherwise post `rows` with the clamped window.
...
- `rows` — an array of `{ i }`, one per index in the window, where `i` is the corpus index of that survivor (the `indices[k]` value). **Interim, index-only:** rich fields (`display`/`score`/`comment`/`wordlist`) and search highlights still come from main's resident corpus (`corpus.entries[i]`, the existing flat-path decode) — the worker doesn't yet own presentation for windowed rows.
