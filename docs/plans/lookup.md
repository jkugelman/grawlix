...
Lookups are the gap-filler for the populations described in [`app.md`](../designs/app.md)'s "workspace and sidekick" framing — specifically constructors whose grid software lacks built-in lookup (Crossfire, Crossword Compiler). Ingrid has Google integration, Crosserville has clue lookup; those users already get lookup elsewhere.
...
- **Where does the lookup result live?** Each option has a different relationship to the always-visible-table decision in [`app.md`](../designs/app.md):
