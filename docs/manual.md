...
**Grouped mode.** The gallery card's Group button clusters entries that share an acronym — every multi-word entry buckets under its word-initial letters, so `the IRS`, `the irate Senator`, and `Tom Is Right` all land in `tis`. Only clusters whose acronym is itself an entry in the wordlist survive (e.g. `tis` shows up only if `TIS` is a row) — the point is bidirectional pairs, not every prefix coincidence. Each cluster row shows the acronym entry as a clickable badged atom (same shape as the chain atoms next to it), so you can adjust its score or comment without leaving the grouped view. The score range applies to the acronym too: drag the range narrower and clusters whose acronym entry falls outside the band drop alongside chains whose atoms do. Sort the grouped view by Acronym (alphabetical), Acronym length, Acronym score, or any of the standard cluster axes (Count, Min/Max score). Single-word entries are skipped (a one-letter "acronym" cluster of every entry starting with a given letter is just a prefix search). Word boundaries are spaces only in this mode — the hyphen-optional branching the filter uses would split each entry across multiple clusters.
