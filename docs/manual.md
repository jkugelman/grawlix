...
**Anagram** returns every entry with the same letters. **Made from letters** keeps every entry you can spell using only the letters you type — a subset is fine and order doesn't matter, so PARENTAL keeps PLANE, RENT, and more. **Palindrome** keeps entries that read the same forwards and back (RACECAR, KAYAK). **Semordnilap** finds words that are reverses of each other (STRESSED ↔ DESSERTS, LIVED ↔ DEVIL). **Behead** and **Curtail** find words that become another word when you drop the first or last letter (SLING → LING, PARTY → PART) — the dropped letter renders with a strike-through; a **Count** field drops more than one. **Search** matches entries by text and wildcards; it's the bar at the bottom, and also a gallery card you can chain mid-stack. Click the caret (`▾`) beside its box to reveal a **Replace** field — fill it and Search turns from a filter into a transform, rewriting each match into another entry (results are kept only when the rewritten word is itself a real entry). **Regex** matches with full regular expressions (`^un.+ed$`); its caret opens a **Replace** field the same way, where `$1`-style backreferences work. Collapsing the caret drops the replacement and the tool is a plain filter again. **Letter clusters** is a different kind of tool — it *groups* the wordlist: give it a letter count and it finds every set of words built from the same distinct letters (POST, STOP, SPOT and TOPS all share `o·p·s·t`).
