...
**Rhymes** narrows the entries table to words that rhyme with what you type — matched on pronunciation, not spelling, so `BLUE` and `THROUGH` rhyme while `THROUGH` and `ROUGH` don't, and a multi-word entry rhymes on its last word (`SPACE OUT` rhymes with `ABOUT`). Like Space out it needs an external dictionary — the CMU Pronouncing Dictionary (~3.5 MB), downloaded and cached the first time you use the tool. In all-mode it clusters your wordlist into rhyme families instead of filtering; a word with more than one pronunciation lands in every family it rhymes into.
