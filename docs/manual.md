...
**The panel is a cross-wordlist provenance view.** Below the editor it lists every wordlist that contributes the clicked entry — in priority order, each with that wordlist's actual entry text, effective score, and comment — **including disabled and non-winning** wordlists. So you can see at a glance whether another list scored or spelled the same word differently, even one you aren't merging. It shows the whole letter-form across every wordlist no matter which spelling you click — `the IRS`, its sibling `Theirs`, and a plain `theirs` all open the same table, listing every spelling from every list. (The merged All Wordlists table does the opposite: it keeps distinct spellings on separate rows. The panel collapses them because it's answering "where does this word live," and they're all the same word.) The table's Sources column gives the same picture at a glance — which lists carry each entry — while this panel is the deep version: the exact spelling, score, and comment each list holds, including disabled and non-winning ones.
