...
- **Alt-↑ / Alt-↓** — with the entry panel open, step to the previous / next entry without closing (also **PgUp / PgDn**, or the **Prev / Next** buttons). Moving saves the current edit. See *Editing entries*.
...
You can select rows and drive the table entirely from the keyboard — built for retiering or deleting a run of entries quickly. **A single click selects a row** — the one exception is the **score badge**, which opens the tier quick-pick — or **click and drag** across rows to select a run. To **open** an entry for editing, **double-click** it; its panel opens with the cursor in the entry field. On a phone, where there's no double-click or drag, a single **tap opens** the entry and multi-select is a desktop-only feature. Once the table has focus, the arrow keys and **PgUp / PgDn / Home / End** move a cursor through the list, **Enter** opens the entry panel (closing it drops you right back on the row, so you can arrow to the next and open that one), and **Esc** clears the selection.
