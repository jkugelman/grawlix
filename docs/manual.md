...
**Suggesting an entry's real-world spelling.** Open a run-together entry (`hasagraspon`) and Grawlix offers its best guess at the spelling, as a **Rename to …** link under the entry box. It fills in fast with plain spacing (**has a grasp on**), then a moment later upgrades itself to the spelling the wider world actually uses — the right capitals, accents, and styling — by checking Wiktionary and Wikipedia: `helenoftroy` becomes **Helen of Troy**, `cafeaulait` becomes **café au lait**. It fixes **already-spaced** entries too, just the casing or accents (`helen of troy` → **Helen of Troy**), and plurals pick up their singular's spelling (`dnasequencers` → **DNA sequencers**). It borrows casing from your own lists too — if you've written `DNA` somewhere, a run-together `dnasplicer` comes back as **DNA splicer** even when nothing online matches. Click the link to drop that spelling into the box — from there it's an ordinary rename, so the preview shows the change and **Save** (or a **Next** step) commits it. Grawlix only ever suggests a respelling of the *same* word, never a different one, so an entry that's already right — or one it can't confirm — gets no suggestion. The link appears only where you can edit: not in a read-only single-wordlist view, and not when adding a brand-new entry. (The fast plain-spacing stage shares the **Space out** tool's word-frequency dictionary, downloaded the first time a suggestion is needed.)

**Looking an entry up.** The panel also carries quick reference look-ups — inline Wikipedia, dictionary, and thesaurus results, plus **Search** links out to Google, OneLook, and XWord Info. When a run-together or oddly-cased entry turns up nothing, Grawlix retries the look-ups against the canonical spelling it worked out above and shows those instead, under a *Showing results for …* note — so `helenoftroy` still surfaces its **Helen of Troy** article.
