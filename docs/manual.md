...
**Anagram** returns every entry with the same letters. **Semordnilap** finds words that are reverses of each other (STRESSED ↔ DESSERTS, LIVED ↔ DEVIL). **Behead** and **Curtail** find words that become another word when you drop the first or last letter (SLING → LING, PARTY → PART) — the dropped letter renders with a strike-through; a **Count** field drops more than one. **Search** matches entries by text and wildcards; it's the bar at the bottom, and also a gallery card you can chain mid-stack. It highlights the literal parts of your query inside each result — the verbatim letters you typed light up in colors, while wildcards (`*`, `?`, `#`, `@`, `[…]`) stay dark, so a substring you searched for stands out at a glance. **Regex** matches with full regular expressions (`^un.+ed$`); fill its **Replace** field and it rewrites matches into other entries instead. It highlights matches the same way — the literal stretches of the pattern light up, or the `(capture groups)` you write yourself. With capture groups, a replacement tints each captured chunk the same on the input and output, so you can watch letters move; without them, the literal parts of your replacement light up too. **Letter clusters** is a different kind of tool — it *groups* the wordlist: give it a letter count and it finds every set of words built from the same distinct letters (POST, STOP, SPOT and TOPS all share `o·p·s·t`). The rest of the gallery is wired up but doesn't transform anything yet — their cards exist so the full catalog stays visible while the tools land.

**Chain rows.** A tool that produces a new word — Behead, Curtail, Semordnilap — shows each result as a stacked row: the original word on top, then each step below it, prefixed with an arrow (`SLING` over `→ LING`). A longer chain stacks further. Semordnilap rows use `↔` because the relationship runs both ways. Click any line to edit that word's score; each search highlights its matches on its own line, so stacking several searches shows the word once per search, one set of highlights apiece.

**Group rows.** Letter clusters produces group rows instead of chain rows: a numbered row showing a **Count** and the cluster's words, each with its score badge. Chain a tool after it and the group grows a line — Search keeps the matching words, Behead or Curtail add a line of what they produce. When a line holds more words than fit, the overflow collapses into a **+N more** chip; click it for a popup with the whole set. Click any word — in the row or the popup — to edit its score. Only one grouping tool fits in a stack at a time.

**Sort.** Axes depend on whether the stack has a transforming tool. With none — a flat list, or just searches — rows sort by Entry / Length / Score. Once a transforming tool is in play, rows sort by Entry / Length / Min score / Max score — Min and Max read across every line of the row. Adding or removing a transforming tool keeps your sort choice rather than resetting it: Entry and Length stay put, Score becomes Min score, and Min or Max score becomes Score. When the primary axis ties, tiebreakers surface the most interesting entry first: longer over shorter, higher-scoring over lower, with alphabetical as the final stable fallback. Flipping asc/desc reverses only the primary axis; tiebreakers keep their direction, so short low-scoring entries don't float to the top of a tied bucket on `score asc`. With Letter clusters in the stack, rows sort by Entry / Count / Min score / Max score.
