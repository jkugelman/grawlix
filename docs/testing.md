...
On macOS `install-deps` is neither needed nor supported; `npx playwright install` is the whole story. macOS also ships no `flock`, but nothing extra is needed for it — [`with-test-lock.sh`](../scripts/with-test-lock.sh) falls back to [`lock-exec.py`](../scripts/lock-exec.py), which locks through `python3` (already required, since that is what serves the site under test).
