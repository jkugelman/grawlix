...
**Sort control inside the search bar.** "Sort by [Entry ▾] [↑]" sits at the right edge of the search bar, after the score-range filter — not in a dedicated toolbar above the table. The search bar's right-hand space is otherwise empty (the search input flex-stretches to fill it), so folding sort in saves a row of vertical space without crowding. Filter (search/whole-word/score) on the left, view-config (sort) on the right; same row.
