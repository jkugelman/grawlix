...
The URL drops `sort=` when the axis matches the current tier's default. A stack edit that flips the tier — adding or removing a transform — never overrides the chosen axis with a tier default; it remaps it. `entry` and `length` exist in both tiers and carry across untouched; `score` ⇄ `min-score`, and `max-score` collapses to `score` when the transform is removed. The tier default applies only on first boot from a URL with no `sort=`, never as a snap-back when crossing the boundary, so the user's sort intent (including the untouched default) survives adding and removing transforms. The sort direction is preserved across the remap. The whole sort story is provisional — expect iteration now that the UI is live.
