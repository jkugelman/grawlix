...
A **group row** (`row.grouped === true`) clusters the merged wordlist rather than transforming it entry by entry. Today's two groupable tools are Anagrams (entries sharing a letter multiset) and Letter bank (entries sharing a distinct-letter set — POST, STOP, SPOT, TOPS cluster under `opst`).

**Grouped is a flag on the stack row, not a separate tool.** A flat tool declares groupability by adding a `group: { key, columns }` sub-object — `key(entry)` returns the equivalence's canonical string, and `columns` declares the per-group attributes that surface in the row. A gallery card for any tool with `group:` sprouts a "Grouped" button; clicking it adds a stack row with the same `tool` key but `grouped: true`. `row.kind()` promotes such a row from the def's static kind to `'group'`, and a grouped row's `params` are ignored — its input is the whole upstream row set. URL representation: `?letter_bank=SPOT` is the flat tool; `?letter_bank&grouped` is the grouped row. `grouped` is a reserved positional keyword — the decoder treats it as a flag on the preceding row when that row's tool declares a `group:` sub-object.

The shared executor helper `bucketize(rows, spec, ctx)` consumes the seeded chain rows and returns **group rows** in their place. A group row is `{ key, lines }`: `key` is the equivalence key (`spec.key(entry)`), and `lines` is a vertical stack of word-set lines. A line is `{ cells, slot }`, a cell is `{ wlEntry, highlights }` — an atom without the relation glyph. `bucketize` produces one line, the cluster itself; clusters of fewer than two members are dropped by the bucketizer, not the executor — a downstream tool may legitimately thin a line to one word, so the executor never imposes the rule.

**Group columns appear as row attributes and sort axes.** Each entry in `group.columns` carries `{ label, value(g), sort?, tiebreaker?, tiebreakers? }`. The label becomes a column header alongside Count; `value(g)` (called with the group object — `key` and `lines` available) renders into the first line of every row. Each column also registers a sort axis (`Sort by Letters`, etc.) unless `sort: false`. When that column is the primary sort, its `tiebreakers` (an explicit `[{ project, dir }, ...]` matching the static axis shape) drives the cascade; if omitted, the fallback is count desc, min score desc, max score desc, then cell entries alphabetical — equal-on-the-column groups surface bigger clusters first, then order by the quality of their worst member, then their best member. The `tiebreaker: false` flag separately opts a column out of contributing to *other* axes' cascades.

**A tool chained after a group tool appends a line** — the group analogue of a chain row appending an atom. `appendGroupLine` maps the group's tail line to a new one: a filter appends the subset of the tail's words that pass; a transform appends the deduped union of its outputs. So Letter bank (grouped) then Behead is a two-line group row — the cluster on line 1, behead's real-word output on line 2. A transform's input marks fold onto the tail line, unless that line already carries another tool's marks, in which case they take a line of their own — the same one-tool-per-highlight-slot rule the chain-row model follows (§ Highlights pipeline). A group row is dropped only when its **last** line empties.
...
**At most one group tool per pipeline.** A second has nothing well-defined to cluster — the first already replaced the entries with group rows — so the gallery disables every Group button once one is in the stack, the stack refuses to add a second, and the URL decoder ignores group tools past the first.
...
**The producing tool's relation glyph prefixes each chained line.** A chained tool's line opens with `def.glyph` (a single symbol — `→` for every shipped transform) rendered inline before the first cell's entry text via the same `.atom-glyph` span chain rows use; tools that declare no glyph (every filter) leave the line bare. One glyph per line, not per cell — the symbol describes the relationship from the prior line to this one, which the whole line shares.
