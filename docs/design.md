...
The merged cache exposes `byNorm` (norm → first row sharing that norm, the lookup tools use) and `byKey` (mergeKey → row, for full disambiguation when needed). My Edits edits go through `applyEditsChange`, which patches just the affected norms' rows into the merged cache in place rather than rebuilding it — see § Caches, *Hot path: editing My Edits*.
...
**Folder layout: outputs at root, originals tucked away.** The root holds what the user feeds their construction software — `grawlix.json`, one `<name> rescored.txt` per wordlist with rescore rules applied, an `All rescored.txt` of the merged-rescored output, `My Edits.txt`, and a `README.md` generated on first setup. The unrescored source files (`<name>.txt`) live in an `original/` subfolder, out of the way of the outputs the user actually points their software at. `My Edits.txt` is the exception that stays at root: it has no rescored variant and is the file the user's software writes back to, so it belongs alongside the outputs. `Disk.readFile`/`writeFile`/`deleteFile` are path-aware (they resolve `/`-separated subdirectories, creating them on write); `rawPathFor(wordlist)` is the single source of truth that maps a wordlist to its raw file's relative path, and the watcher recurses into `original/` so external edits to source files still route back.
...
**Cross-device via OS sync, not Grawlix code.** Grawlix ships no cloud code. Cross-device works because the user's existing cloud-drive client (Dropbox, iCloud Drive, OneDrive, Google Drive) syncs the folder. The user picks a synced folder on device A; on device B, the **Load existing** card points at the same path. The diff-only merge dialog handles whatever's already there. The `README.md` Grawlix writes on first setup names this workflow explicitly so users discover it without being told.
...
Construction software disagrees on what entries it can read — Ingrid and Crossword Compiler take rich entries (spaces, accents, punctuation), Crosserville forbids special characters. A single global **output format** (`mergedSettings.outputFormat`) decides how Grawlix *writes* entries, governing both downloads and the disk-storage fill outputs at once. It's expressed as independent keep-or-strip axes rather than named modes — booleans for `spaces`, `punctuation`, `accents`, and `comments` — so it mirrors the orthogonal knobs real software exposes ("strip accents", "ignore special characters") without Grawlix hardcoding any one program's quirks. The default is fully rich (everything kept), byte-for-byte identical to the pre-format disk output, so existing folders are untouched until the user opts to strip something. (Named presets like "Crosserville-ready" were rejected — they'd need verified per-program facts; the raw axes stand on their own. A letter-case axis was considered and cut — case carries no crossword meaning and no one's asked; entries write in their existing case, plain entries already lowercase via `norm`.)
...
**Fill outputs vs. editable/source files.** The format applies only to *fill outputs* — the rescored per-source files, `All rescored.txt`, and the rescored/All downloads, the files the user points construction software at. *Editable and source files* are always written as-is with comments (the `AS_IS_FORMAT` default): `My Edits.txt` (the file the user writes into and Ingrid round-trips), the `original/*.txt` source files, and the My Edits / original downloads. Normalizing `My Edits.txt` would destroy the rich entries the user deliberately typed and fight external editing; its entries still reach letters-only software through `All rescored.txt`, which is a fill output. The format lives in `mergedSettings` (localStorage in IDB mode, `grawlix.json` in disk mode), so a cloud-synced folder stays format-consistent across devices instead of two machines rewriting `All rescored.txt` against each other on boot — and a folder-load uses the *folder's* format, not the loading device's. Changing the format calls `applyOutputFormat`, which persists it and regenerates the fill outputs on disk (a no-op in IDB mode).

**Collapse on strip.** `formatEntryText` is many-to-one once any stripping is active — `café`/`cafe` collide when accents drop, `co op`/`coop` when spaces drop. `serializeEntries` collapses colliding output strings: the highest score wins (matching the merge's "highest rescored value wins") and distinct non-blank comments combine, deduped and joined by ` / `. Combining rather than picking a winner is deliberate — within one already-merged list these rows are peers with no priority order (often the same score), so dropping a comment would be arbitrary. The fully-rich path skips the collapse and writes verbatim, so raw/editable files round-trip exactly.

**Editing: Settings is the sole owner.** The format's default is edited in one place — the Settings dialog (shared `buildOutputFormatControlsHTML` / `readOutputFormatControls` / `wireOutputFormatControls` helpers). Each checkbox click persists the setting immediately (`setOutputFormat`), the same live-apply model as the dialog's other settings, so downloads pick it up at once and a user watching the folder sees it react. The expensive part — `regenerateFillOutputs` re-serializing every rescored file plus `All rescored.txt` — is split out and debounced ~1s (`OUTPUT_FORMAT_REGEN_DELAY`), flushed on close, so a burst of checkbox clicks coalesces into one rewrite instead of one per click. `regenerateFillOutputs` is single-flighted (a change landing mid-rewrite re-runs once at the end) so overlapping regenerations can't race the same files or leave them stale at the old format. The download dialog — the one place a one-off deviation is useful (handing a Crosserville file to a friend while your own folder feeds Ingrid) — seeds the same editable controls from the global format: **Download** serializes with whatever's on screen *transiently*, never touching the global or disk; a **Save as default** button appears only when the on-screen format diverges, and clicking it routes through `applyOutputFormat`. Download stays a predictable "give me this file now"; promoting a tweak to the default is a separate, deliberate act rather than a modal interruption.
...
**Column headers drive the same axes**, a click-to-sort shortcut layered over the Sort-by dropdown — which stays the definitive, complete control. Each header column owns an ordered set of axes (canonical first), filtered at render time to the axes the current tier actually has; a click flips direction when the active axis is one the column owns, otherwise it jumps to the column's canonical axis ascending (`nextSortForColumn` over a `data-sort-axes` attribute `buildEntryHeadersHTML` stamps onto each cell, dispatched by a delegated listener on `.sticky-stack`). The bindings: Entry/Len/Score map to `entry`/`length`/`score`, except the Score column owns `min-score` (canonical) plus `max-score` once a transform promotes the tier; in a grouped pipeline Count owns `count`, each group column owns its own key, and the anchor header (Initialisms) owns `entry`/`length`/`score`. The mapping is deliberately partial: `min-score`/`max-score` in a grouped pipeline have no column — group rows carry no score column — so they stay dropdown-only, and a multi-axis column (Score) neither says which of its axes is active by its arrow nor cycles between them on a click (it flips the active one; the dropdown reaches the rest, so Max score is dropdown-then-flip). First-click direction is always ascending, uniform across columns, for predictability. The arrow marks whichever column owns the active axis; the shared `applySort` rebuilds the headers on every sort change so the dropdown and arrow never disagree. Headers are keyboard-focusable and Enter/Space activates, with focus restored across the rebuild.
...
- **2D reading.** A table lets you sort by one axis and visually scan another (sort by Min, eyeball Max). The list can't — switching axes is one click away (the sort control or a column header — § Sort axes per tier), but you see one ordering at a time. In practice users sort by their primary axis and scroll; switching is rare.
...
| `wordlist._rescoredByNorm` | per-wordlist | `_rescored` (`norm` → every wlEntry for that norm; variant-aware, feeds single-norm merge rebuilds) | `invalidateRescoredCache(wordlist)` |
...
| `_mergedWordlistCache` | module | every enabled wordlist's `_rescored` (entries + `byNorm` + `byKey` maps) | `invalidateSourceCounts()`; or patched in place per-norm by `patchMergedForNorms` on My Edits edits |
| `_sourceCountsCache` | module | aliases `_mergedWordlistCache.sourceCounts` | `invalidateSourceCounts()` |
| `_mergedWordlistCache._initialChains` | module (on the merged cache) | the cache's `entries` — one seed chain row each | replacing `_mergedWordlistCache`; spliced in parallel by `patchMergedForNorms` |
...
**Hot path: editing My Edits.** Score and comment edits, new-entry adds, and deletes all route through `applyEditsChange(edits, norms, mutate)`. It snapshots the merged winners for the affected norms (an entry-text rename touches two — the old norm and the new), applies the mutation, drops My Edits' own derived caches, then calls `patchMergedForNorms` to splice just those norms' recomputed rows into `_mergedWordlistCache` in place. Each norm's bucket is rebuilt by `computeMergedBucket` — an O(sources) scan of every enabled wordlist's `_rescoredByNorm` index — so a single edit costs O(sources), not the O(all-entries) of a full rebuild. The cache object survives, so its `_initialChains` seed and `sourceCounts` are patched in parallel rather than discarded, and `refreshWorkshopMergedScroller` re-runs the pipeline against the patched cache (reusing the seed). The alternative — `invalidateSourceCounts()` on every keystroke — forces a ~700ms rebuild of a 500K-entry merged view, a visible freeze; that was the behavior for a stretch after the Library/Workshop redesign dropped an earlier in-place patch, until this one restored it. `computeMergedBucket` must reproduce `buildMergedWordlist`'s per-bucket logic exactly — including deduping winners by contributor for `sourceCounts`, and honoring `(norm, display)` variants — or the patched cache silently diverges from a rebuild. Two tests pin it: one asserts the cache object is patched in place (not replaced), the other that the patched result is byte-for-byte a full rebuild.
