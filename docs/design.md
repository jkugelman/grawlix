...
**The find/replace widget.** A tool flagged `findReplace: true` — Search and Regex — renders its `pattern` and `replace` inputs as a caret-expandable widget (`buildFindReplaceFieldHTML`): the pattern input alone with a caret button to its left, the replace input stacked directly below it and hidden until the caret expands the row. It looks the same at every width — no side-by-side layout, no breakpoint. Expansion is a transient `row._replaceExpanded` flag, never URL-serialized; a row also renders expanded whenever its `replace` param is non-empty, so a shared link carrying a replacement opens already expanded. Collapsing the caret clears `params.replace` and drops it from the URL — a collapsed row is always a pure filter — but the replace `<input>` is hidden, not destroyed, so re-expanding restores its text. Both Search and Regex are filter/transform hybrids — `kind: params => params.replace ? 'transform' : 'filter'` — so an expanded-but-empty row still behaves as a plain filter. Inputs are placeholder-labelled, not `<label>`-prefixed. Every other tool's params render through the same `buildParamsHTML`, which groups text and number inputs in a `.tool-row-fields` flex group with checkboxes beside it.
