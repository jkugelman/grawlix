...
**Sticky region: stats bar → tool stack (when populated) → search bar.** Stats joins the sticky region because the histogram is a clickable filter affordance — keeping it reachable while scrolling the word list is worth the extra row of sticky chrome. The tool stack is hidden when empty so pre-tool-use Grawlix looks unchanged — see *Tool gallery & stack* below.
