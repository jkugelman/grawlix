...
**Group columns appear as row attributes and sort axes.** Each entry in `group.columns` carries `{ label, value(g), sort?, tiebreaker? }`. The label becomes a column header alongside Count; `value(g)` (called with the group object — `key` and `lines` available) renders into the first line of every row. Each column also registers a sort axis (`Sort by Distinct letters`, etc.) unless `sort: false`, and contributes to the tiebreaker cascade unless `tiebreaker: false`. Tiebreaker order follows the columns array; the cascade after a user-selected primary is: column-derived tiebreakers (in declared order) → the existing per-axis tiebreakers → group key alphabetical.
