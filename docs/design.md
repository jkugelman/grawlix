...
Tools live in two places: a persistent **gallery** as a top section of the card, and a **tool stack** inside the sticky region just below the brand header. The gallery is browseable; the stack is the user's current pipeline. The chrome, the pipeline runtime, and six tools with working logic — Anagram, Semordnilap, Behead, Curtail, Search, and the group tool Letter clusters — are shipped; the rest of the catalog renders gallery cards but doesn't yet produce results. The remainder of the catalog and chaining extensions are tracked in [`planned/tools.md`](planned/tools.md). Tool output lands in the entries table (§ Entries table) as **chain rows** (§ The chain-row model) — or, with a group tool in the stack, as **group rows** (§ The group-row model).
...
**Cheat-sheet popovers are a per-param opt-in.** A tool param declares an optional `help` HTML string on its schema; `buildHelpHTML` produces the standard markup — a `<kbd>`-and-description grid plus an optional footer link — but `help` accepts any HTML. At load time `PARAM_HELP` is built from every param that declares `help`, keyed `toolKey/paramKey`. The input builders emit a matching `data-help` attribute — `paramsHTML` for generic tool rows, `buildSearchInputRowHTML` for the three Search surfaces — and `attachHelpPopups` binds a `PopupHelp` popover to *every* `data-help` input found by a document-wide scan, resolving its content through `PARAM_HELP`. It runs whenever a view re-renders and an anchor may have been rebuilt — `mountWorkshopPanel`, `rerenderRows`, and `LibraryView.renderPane` — destroying the prior popovers and rebinding from scratch each time. Search's `query` param carries the wildcard cheat sheet; Regex's `pattern` and `replace` params carry regex-syntax sheets (no cheat sheet covers all of regex, so both link out to regexone.com). The popover is interactive — a `mousedown`-preventDefault keeps the anchored input focused — so a click on the footer link lands instead of blurring the input and dismissing the popover first.
...
The `mergedWordlist` it receives is already trimmed to the score-range control's band: the score range is a **pre-pipeline filter**, not a view filter on the output, so out-of-range words never enter the row set and are invisible to every tool's lookups. Trimming upstream rather than filtering the output is what lets group rows behave: a cluster with one out-of-band member would otherwise vanish wholesale under a range that member fails, since a group's score spans every member. `workshopPipelineWordlist()` derives the trimmed view (memoized on the merged cache); a no-range control passes the merged cache through untouched.
...
**Atom count is static.** Every row in a given pipeline has the same atom count, derivable from the catalog records alone — `currentAtomCount(stack)` simulates the executor's emit-then-unify over the active tools (run-bearing, not inert) without inspecting any row. The originator is one atom; each transform appends a new-word output atom; each highlighting tool (a search, or a transform marking its input via `inputHighlights`) appends a same-word atom that `unify` folds into the tail unless the tail is itself a highlight slot — so the originator and the first search collapse to one atom, while three searches on one word leave three. `atomCount` is the resulting count; the renderer and the virtual scroller read it, so the scroller knows every row's height (`atomCount` × row height) without measuring. An empty Search row is *inert* — it reports `isInert(params)` and is skipped like a run-less tool, so an empty search bar adds no atom.
...
A **group tool** (`kind: 'group'`) clusters the merged wordlist rather than transforming it entry by entry. `letter_clusters` is the first and so far only one: it groups entries by their set of distinct letters — POST, STOP, SPOT, TOPS cluster under `opst` — keyed by a "Distinct letters" count.

A group tool's `group(rows, prepared, ctx)` consumes the seeded chain rows and returns **group rows** in their place. A group row is `{ key, lines }`: `key` identifies the cluster (for `letter_clusters`, the sorted distinct-letter string), and `lines` is a vertical stack of word-set lines. A line is `{ cells, slot }`, a cell is `{ wlEntry, highlights }` — an atom without the relation glyph. `letter_clusters` produces one line, the cluster itself; clusters of fewer than two members are dropped by the tool, not the executor — a downstream tool may legitimately thin a line to one word, so the executor never imposes the rule.

**A tool chained after a group tool appends a line** — the group analogue of a chain row appending an atom. `appendGroupLine` maps the group's tail line to a new one: a filter appends the subset of the tail's words that pass; a transform appends the deduped union of its outputs. So `letter_clusters` then `behead` is a two-line group row — the cluster on line 1, behead's real-word output on line 2. A transform's input marks fold onto the tail line, unless that line already carries another tool's marks, in which case they take a line of their own — the same one-tool-per-highlight-slot rule the chain-row model follows (§ Highlights pipeline). A group row is dropped only when its **last** line empties.

This append-a-line model replaced an earlier "flatten / run / unflatten" sketch, where each chained tool ran as a sub-pipeline over every group's members and the group re-formed around the survivors. Appending a whole line is simpler and composes identically — each tool contributes one line, the group stays group-shaped, and there are no per-member sub-chains to track.

**At most one group tool per pipeline.** A second has nothing well-defined to cluster — the first already replaced the entries with group rows — so the gallery disables every group card once one is in the stack, the stack refuses to add a second, and the URL decoder ignores group tools past the first.
...
`unify` runs only when the stack actually produces multi-atom rows — when some active stage is a transform or a highlighting filter. A filter-only chain leaves every row a lone atom, where both the mirror collapse and the within-row collapse are no-ops, so the executor skips the pass and returns its rows untouched.

### Chain-row display

Atoms render top-to-bottom on every viewport — there is no side-by-side layout at any width. The entries table is one CSS Grid per row with `grid-auto-rows`; each atom occupies one grid line (`count`, then `entry` / `len` / `score`) — except an atom that repeats the word above it (a search's same-word atom), which renders entry-only — and lines past the first wrap onto their own grid row while staying column-aligned with the line above via shared `--entry-w` / `--len-w` / `--score-w` variables. The relation glyph prefixes every word-introducing atom's entry; a repeat atom carries none. Row stride is atom count × row height; the virtual scroller reads the static atom count for its stride math, and the row's own height is content-driven by the grid.

Comment and Source columns appear on every chain shape — a one-atom row and a stacked multi-atom row alike — when the viewport has room, dropping staggered as it narrows (source first at <960px, then comment at <760px). They render per-atom: each atom line carries its own comment and source, so a chain row reads top-to-bottom as where every word in the journey comes from. A synthetic atom — built from a tool's `[string, score]` output, sourced from no wordlist — gets blank cells. Column visibility is pure CSS media-query gating; the renderer emits the cells on every full atom line, while a repeat atom omits comment and source along with its len and score. Multi-atom rows have the horizontal room because each atom line carries only `entry / len / score`. Headers stay constant: the Entry / Length / Score labels describe what each *line* contains, not the row, so one header set serves every chain shape.

### Group-row display

A group row stacks its lines the way a chain row stacks atoms, but the columns differ: each line shows a **Count** — its number of words — and the **Entries**, a horizontal run of word-and-score-badge cells, with the row number leading the first line only. There is no Length, Comment, or Source column; a group is about set membership, and per-word metadata would have to ride inside each cell — a denser layout deferred with the rest of the group polish. The line count is static across a pipeline, so the virtual scroller keeps a fixed per-row stride.

**Overflow truncates rather than scrolls.** A line wider than its column clips, ending in a `+N more` chip — `N` the count of words past the edge — with a fade ramping the last visible word out before the chip. Clicking the chip opens a popover with the line's full word set. An earlier design scrolled each line horizontally in its own container; truncation won because independent per-line scrollbars desynced a group's lines, and hiding the scrollbars left no affordance. A group member is editable wherever it shows — a visible cell or a word in the reveal popover — through the same AtomPopover the chain rows use. The reveal popover dismisses on Escape, an outside click, or a re-click of its chip; like the AtomPopover it rides out scroll rather than closing, so opening an editor on a hidden word (which scroll-into-views on iOS) can't pull it down.
...
In a grouped pipeline the axes project off the whole group: Entry off the cluster line's words in display order, Count off the member count, Min/Max score across every word in every line. The words *within* a group line are themselves ordered to match the axis — alphabetical under Entry, score-descending otherwise — and Entry holds them ascending regardless of the row direction, since the toggle reverses the rows, not the words inside a cluster.
...
**Search and Regex highlights** mark pattern matches in the entry slot via `<mark>` spans, colored per fixed segment or capture group. Ellipsis truncation respects the markup.
...
**Compact mode when the bar can't fit.** The bar never wraps. When it's too narrow to show `.search-bar-end` (the score-range filter plus sort control) inline, `SearchBarCompact` collapses that group behind an adjustments button at the right edge; clicking it opens a popover. The group is *moved* into the popover, not duplicated — so the score/sort inputs keep their unique ids and the handlers wired into them (notably the sort `<select>` the entries scroller mounts into `#search-bar-sort`) stay intact. The collapse is overflow-driven, not a fixed breakpoint: a ResizeObserver re-measures the *expanded* layout (`.compact` stripped) for horizontal overflow, so the bar reacts to its own width after the wordlist rail takes its slice — the same approach the stats bar's `Mean · Median · Mode` overflow uses.
