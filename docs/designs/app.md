...
per-wordlist navigation, no sync state, no wordlist picker.

**Left rail.** Fixed-width column with two labelled sections: a **Wordlist**
section at the top (wordlist dropdown + sync indicator — the two doors into
...
**Wordlist dropdown.** Top of the rail. Pure picker. First entry is `All` —
the merged result of every enabled wordlist with rescore rules applied. Below
it, every individual wordlist in `state.sources` order (My Edits at the top
...
wordlist view (merged All, individual wordlists, future tool results). Edits
...
Manage wordlists never see it; the defaults are sensible enough that this
is fine.
...
- All or My Edits → Sync & backup dialog (it's a backup, not just a save —
  ...
  lands).
- Any individual wordlist → Manage wordlists dialog → that wordlist's Download
